...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.op === 'snapshot') {
      ...
      if (hasAdminMirror()) {
        try {
          const admin = await getAdmin(); const collection = admin.firestore().collection('secureVault').doc(uid).collection('items'); const existing = await collection.get();
          ...
          const chatId = getUserByUid(uid)?.telegramChatId; if (chatId) await admin.firestore().collection('telegramLinks').doc(chatId).set({ uid, updatedAt: Date.now() }, { merge: true });
    ...
    if (body.op === 'delete') removeRuntimeItem(uid, id);
    else { const serialized = JSON.stringify(body.item || {}); if (serialized.length > 100000) return res.status(413).json({ error: 'Record too large' }); putRuntimeItem(uid, body.item); }

    if (!hasAdminMirror()) return res.status(200).json({ ok: true, mirrored: 'runtime' });
    try {
      const admin = await getAdmin(); const ref = admin.firestore().collection('secureVault').doc(uid).collection('items').doc(id);
