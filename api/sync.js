  ...
  try {
    ...
    if (body.op === 'snapshot') {
      ...
      if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON && process.env.VAULT_SERVER_KEY) {
        ...
        const chatId = String(process.env.TELEGRAM_CHAT_ID || ''); if (chatId) await admin.firestore().collection('telegramLinks').doc(chatId).set({ uid: identity.uid, updatedAt: Date.now() }, { merge: true });
    ...
    if (!process.env.FIREBASE_SERVICE_ACCOUNT_JSON || !process.env.VAULT_SERVER_KEY) return res.status(200).json({ ok: true, mirrored: 'runtime' });
