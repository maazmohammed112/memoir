...
export async function getAdmin() {
  ...
  adminFacadePromise = (async () => {
    const [{ cert, getApps, initializeApp }, { getAuth }, { getFirestore }, { getDatabaseWithUrl }] = await Promise.all([
    ...
    const databaseUrl = process.env.FIREBASE_DATABASE_URL || `https://${options.projectId}-default-rtdb.firebaseio.com`;
    return { app, auth: () => getAuth(app), firestore: () => getFirestore(app), database: () => getDatabaseWithUrl(databaseUrl, app) };
  })().catch(error => {
...
export async function verifiedSessionFor(identity, deviceId) {
  ...
  try {
    ...
    const expiresAt = parseExpires(session);
    if (session && (!currentDeviceHash || session.deviceHash === currentDeviceHash) && expiresAt > now) {
      return { ...session, id: sessionId, expiresAt, expiresAtMs: expiresAt };
    }
  } catch (error) {
    console.warn('Realtime auth session lookup failed, checking Firestore:', error?.message || error);
  }
