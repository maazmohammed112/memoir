...
export async function saveVaultDocument(uid, { data, mimeType = 'application/pdf', fileName = 'document', source = 'app', createdAt = Date.now() }) {
  ...
  if (isR2Configured()) {
    try {
      ...
      console.warn('R2 document upload failed, falling back to Firestore:', r2Err?.message);
    }
  }
...
export async function deleteVaultDocument(uid, assetId) {
  ...
  if (snapshot.exists) {
    ...
    for (let i = 0; i < chunks.docs.length; i += 100) {
      await Promise.all(chunks.docs.slice(i, i + 100).map(d => d.ref.delete()));
    }
    await docRef.delete();
    return true;
  }
