  ...
  try {
    ...
    try {
      ...
      if (rtdbSnapshot.exists()) {
        const value = rtdbSnapshot.val() || {};
        rawDocs.push(...Object.entries(value).map(([id, data]) => ({ ...(data || {}), id })));
      }
    } catch (rtdbError) {
      console.warn(`Realtime Database vault read failed for ${uid}:`, rtdbError?.message || rtdbError);
    }

    // 2. Recovery read from Firestore only when RTDB has no vault data.
    if (!rawDocs.length) {
      try {
        const snapshot = await admin.firestore().collection('secureVault').doc(uid).collection('items').get();
        if (snapshot && !snapshot.empty) rawDocs.push(...snapshot.docs.map(document => ({ ...document.data(), id: document.id })));
    ...
    // 3. Last recovery source: client-encrypted RTDB documents.
    if (!rawDocs.length) {
      try {
        const userSnap = await admin.database().ref(`users/${uid}/items`).get();
        if (userSnap.exists()) {
          const val = userSnap.val() || {};
          rawDocs.push(...Object.entries(val).map(([id, data]) => ({ ...(data || {}), id })));
        }
      } catch {}
    }
