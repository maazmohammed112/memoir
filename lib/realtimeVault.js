...
export async function readDecryptedVaultItems(uid) {
  ...
  try {
    ...
    try {
      ...
      // Fallback: Read from Realtime Database
    ...
    if (!docs.length) {
      // Fallback 2: Check users collection in Realtime Database
      const userSnap = await admin.database().ref(`users/${uid}/items`).get().catch(() => null);
      if (userSnap && userSnap.exists()) {
        docs = Object.values(userSnap.val() || {});
      }
    }
