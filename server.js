...
server.get('/api/health', (_req, res) => res.json({ ok: true, gemini: Boolean(process.env.GEMINI_API_KEY), mistral: Boolean(process.env.MISTRAL_API_KEY), telegram: Boolean(process.env.TELEGRAM_BOT_TOKEN), telegramPolling: Boolean(process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID), secureMirror: Boolean(process.env.FIREBASE_SERVICE_ACCOUNT_JSON && process.env.VAULT_SERVER_KEY) }));
