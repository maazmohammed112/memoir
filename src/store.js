...
const FIREBASE_AUTH_TIMEOUT_MS = 45000;
  ...
  async signIn(email, password) {
    ...
    try {
      ...
      this.session = {
        ...
        otpExpiresAt: Number(result.expiresAt || Date.now() + 5 * 60 * 1000),
        otpResendAt: Number(result.nextRequestAt || Date.now() + 90 * 1000),
        otpRequestsRemaining: Number(result.remainingRequests ?? 5),
